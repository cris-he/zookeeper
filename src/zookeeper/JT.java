...
import zookeeper.JobTracker.RunListenForClients;
import zookeeper.JobTracker.RunListenForTasks;
import zookeeper.JobTracker.UseCount;
...
public class JT {
  ...
  static String ZK_TASKS =
      "/tasks"; // like an event queue, task can be submit or query
  static String ZK_JOBS =
      "/jobs"; // for submit tasks (jobs) only, used by worker
  ...
  private void initZNodes() {
    ...
    try {
      ...
      if (status == null) {
        zk.create(ZK_TRACKER, null, ZooDefs.Ids.OPEN_ACL_UNSAFE,
                  CreateMode.PERSISTENT);

        mode = TRACKER_PRIMARY;
        myPath = ZK_TRACKER + "/" + TRACKER_PRIMARY;
        zk.create(myPath, null, ZooDefs.Ids.OPEN_ACL_UNSAFE,
      ...
      if (zk.exists(ZK_CLIENTS, false) == null) {
        zk.create(ZK_CLIENTS, null, ZooDefs.Ids.OPEN_ACL_UNSAFE,
                  CreateMode.PERSISTENT);
      ...
      if (zk.exists(ZK_WORKER, false) == null) {
        zk.create(ZK_WORKER, null, ZooDefs.Ids.OPEN_ACL_UNSAFE,
                  CreateMode.PERSISTENT);
      ...
      if (zk.exists(ZK_TASKS, false) == null) {
        zk.create(ZK_TASKS, null, ZooDefs.Ids.OPEN_ACL_UNSAFE,
                  CreateMode.PERSISTENT);
      ...
      if (zk.exists(ZK_JOBS, false) == null) {
        zk.create(ZK_JOBS, null, ZooDefs.Ids.OPEN_ACL_UNSAFE,
                  CreateMode.PERSISTENT);
      ...
      if (zk.exists(ZK_RESULTS, false) == null) {
        zk.create(ZK_RESULTS, null, ZooDefs.Ids.OPEN_ACL_UNSAFE,
                  CreateMode.PERSISTENT);
  ...
  public class RunListenForClients implements Runnable, Watcher {
    ...
    public void listenForClients() throws KeeperException, InterruptedException,
  ...
  public class RunListenForTasks implements Runnable {
    ...
    public void listenForTasks() throws KeeperException, InterruptedException,
                                        UnsupportedEncodingException {
      List<String> tasks;
      Stat status;
      String dataStr;
      ...
      while (true) {
        ...
        for (String path : tasks) {
          ...
          if (status != null) {
            ...
            // debug(String.format("task in path %s/%s is %s", ZK_TASKS, path,
            // dataStr));
  ...
  private void handleTask(TaskPacket p, String path) {
    ...
    if (p.packet_type == TaskPacket.TASK_SUBMIT)
      handleJob(p, path);
    else if (p.packet_type == TaskPacket.TASK_QUERY)
      handleQuery(p, path);
    else
  ...
  private void handleQuery(TaskPacket p, String path) {
    ...
    try {
      ...
      if (stat != null) {
        ...
        if (tokens[0].equals(
                "success")) { //} else if (tokens[0].equals("success")) {
          response = "password found: " + tokens[1];
        } else if (tokens[0].equals("fail")) {
          response = "password not found; try again."; // response = "error: job
                                                       // could not be
                                                       // processed";
      ...
      debug(String.format("adding result '%s' to path %s", response,
                          clientResponsePath));

      String res = zk.create(clientResponsePath, response.getBytes(),
    ...
    try {
      ...
      if (statJob == null) {
        ...
        if (!clientJobs.get(p.c_id).contains(p.hash)) {
          ...
          UseCount ucount = new UseCount(jobPath);
          debug(String.format(
              "job for %s exists with usecount %d, incrementing count", p.hash,
              ucount.count));
  ...
  private void addJobToMap(TaskPacket p) {
    if (clientList.contains(p.c_id)) {
      if (clientJobs.get(p.c_id) == null) {
        clientJobs.put(p.c_id, new ArrayList<String>()); // no ArrayList
                                                         // assigned, create new
                                                         // ArrayList
      }
  ...
  public void process(WatchedEvent event) {
    ...
    try {
      ...
      if (mode.equals(TRACKER_BACKUP) // primary failure handling
          && isNodeDeleted && nodeName.equals(TRACKER_PRIMARY)) {
  ...
  public class UseCount {
    ...
    public boolean incrementUseCount(String jobPath)
  ...
  public static void main(String[] args)
    ...
    if (args.length == 2) {
      ...
      // String addrId = String.format("%s:%d",
  ...
  private static void debug(String s) {
    ...
    if (debug && mode != null) {
      System.out.println(
          String.format("TRACKER_%s: %s", mode.toUpperCase(), s));
    } else {
