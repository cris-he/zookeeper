...
// Try and find if the password hash is equal to the word hash in worker's
...
public class WorkerHandler extends Thread {
   ...
   * @param args
   */
  public WorkerHandler(ZkConnector zkc, String path, String w_id_string)
      throws IOException {
    super("WorkerHandler");
  ...
  private void registerToJob() {
    ...
    try {
      ...
      String p;
      p = zk.create(path + "/" + w_id_string, // Path of znode
  ...
  private boolean getDictPartition() {
    try {
      debug("getDictPartition: partition_id = " + partition_id +
            " numWorkers = " + numWorkers);

      // Create dictionary request packet
      packet = new PartitionPacket(PartitionPacket.PARTITION_REQUEST,
                                   partition_id, numWorkers);
  ...
  public void run() {
    ...
    while (true) {
      ...
      // Keep polling
      boolean isNewMembers =
          checkNewMembers(); // Check if new members have entered
      if (isNewMembers) {
